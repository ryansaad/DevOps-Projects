...
# 4. Private App Subnets (App Tier)
...
# 5. Private DB Subnets (Data Tier)
...
# 7. Route Tables
# A. Public Route Table (Traffic -> IGW)
...
# B. Private Route Tables (Traffic -> Local NAT GW)
# We create one RT per AZ so subnets use the NAT GW in their own zone.
